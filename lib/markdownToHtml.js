...
const htmlParser = () => (tree) => {
  ...
  headersElements.forEach((header) => {
      ...
      const text = (textNode && textNode.value) || '-empty-';
      const id = text.toLowerCase().replace(/\W/g, '-');
      header.properties.id = id;
  });

  const links = findNodes(nodes, node => node.tagName === 'a');
  links.forEach(link => {
    ...
    if (href.endsWith('.mp4')) {
      ...
      // Replace the <a> link with the <video> tag
      Object.assign(link, videoTag);
    }
  });
};
